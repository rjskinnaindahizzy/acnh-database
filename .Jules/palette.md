...
## 2026-02-06 - [Table Density Control]
**Learning:** Fixed-layout tables often truncate content, frustrating users who need to read full text. Forcing full text globally breaks layout. A user-controlled toggle offers the best of both worlds.
**Action:** Implement "Wrap Text" toggles for data-heavy tables, persisting the preference in `localStorage` so users don't have to reset it every session.
