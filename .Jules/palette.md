...
## 2026-01-28 - [Empty State Focus Recovery]
**Learning:** When interactive elements (like a "Clear Filters" button in an empty state) are removed from the DOM upon interaction, the keyboard focus drops to the document `body`. This breaks the keyboard navigation flow.
**Action:** Always explicitly programmatically shift focus (e.g., to the search input) when removing the currently focused element from the DOM to maintain a continuous, logical interaction flow.
